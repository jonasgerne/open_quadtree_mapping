...
quadmap::SeedMatrix::SeedMatrix(
        ...
        : width(_width), height(_height), cost_downsampling(_cost_downsampling), camera(cam),
          depth_output(_width, _height), debug_image(_width, _height), epipolar_image(_width, _height)
        //income
        , income_image(_width, _height), pre_income_image(_width, _height), income_gradient(_width, _height)
        //copy lsd
        , keyframe_semidense(_width, _height), keyframe_image(_width, _height), keyframe_gradient(_width, _height),
          semidense_on_income(_width, _height), pixel_age_table(_width, _height), depth_fuse_seeds(_width, _height),
          initialized(false), doBeliefPropagation(doBeliefPropagation), useQuadtree(useQuadtree), doFusion(doFusion),
          doGlobalUpsampling(doGlobalUpsampling), fixNearPoint(fixNearPoint), printTimings(printTimings), P1(P1),
          P2(P2), inverse_depth(inverse_depth), min_depth(min_depth), max_depth(max_depth),
          new_keyframe_max_angle(new_keyframe_max_angle), new_keyframe_max_distance(new_keyframe_max_distance),
          new_reference_max_angle(new_reference_max_angle), new_reference_max_distance(new_reference_max_distance),
          min_inlier_ratio_good(min_inlier_ratio_good), min_inlier_ratio_bad(min_inlier_ratio_bad),
          new_variance_factor(new_variance_factor), prev_variance_factor(prev_variance_factor),
          variance_offset(variance_offset), frame_index(0) {
...
bool quadmap::SeedMatrix::input_raw(cv::Mat raw_mat, const SE3<float> T_curr_world) {
    ...
    input_image = raw_mat;
    unsigned char seq = raw_mat.at<unsigned char>(0, 0);
...
bool quadmap::SeedMatrix::add_frames(
    ...
    // Comment Niko: If not a dense frame, return update with semidense depth
    // Comment Jonas: outputs depth maps only at the semi2dense_ratio share of incoming images, e.g. semi2dense_ratio 3: 30 Hz -> 10 Hz
    if (frame_index % semi2dense_ratio != 0) {
        // check if the distance between the latest keyframe and the incoming frame is to big or
        // the angle is smaller than the maximum new angle, if yes, append the current frame to the framelist for multi-view-stereo
        if (need_add_reference())
            add_reference();
...
void quadmap::SeedMatrix::create_new_keyframe_async() {
    ...
    image_grid.x = (width + image_block.x - 1) / image_block.x;
    image_grid.y = (height + image_block.y - 1) / image_block.y;
    regulizeDepth_kernel << < image_grid, image_block, 0, swict_semidense_stream1 >> >
                                                          (keyframe_semidense.dev_ptr, true);
    regulizeDepth_FillHoles_kernel << < image_grid, image_block, 0, swict_semidense_stream1 >> >
                                                                    (keyframe_semidense.dev_ptr);
    regulizeDepth_kernel << < image_grid, image_block, 0, swict_semidense_stream1 >> >
                                                          (keyframe_semidense.dev_ptr, false);
    cudaStreamSynchronize(swict_semidense_stream1);
...
void quadmap::SeedMatrix::extract_depth() {
    ...
    for (int i = 0; i < framelist_host.size(); i++) {
        ...
        printf("   keyframe_seq: %f, seq: %f, transform: %lf %lf %lf\n", income_seq, this_ele.seq,
               gpu_ele.transform(0, 3), gpu_ele.transform(1, 3), gpu_ele.transform(2, 3));
    }
    match_parameter.current_frames = framelist_host.size();
    match_parameter.setDevData();
#ifdef DEBUG_PRINT
    for (int i = 0; i < match_parameter.current_frames; ++i){
        framelist_host[i].frame_ptr->getDevData(reinterpret_cast<float *>(cv_look[i].data));
        char buffer[256];
        sprintf(buffer, "/home/jonasgerstner/Pictures/look-%010d.png", int(framelist_host[i].seq));
        cv::imwrite(buffer, cv_look[i]);
    }
    ...
    if (useQuadtree) {
        ...
        if (printTimings)
            printf("  quadtree cost %f ms \n", (std::clock() - depth_extract_start) / (double) CLOCKS_PER_SEC * 1000);
        depth_extract_start = std::clock();
    }
    ...
    /* CUDA Kernerls: <<<Dg,Db,Ns,S>>>, where:
    ...
    */
    ...
    if (!doBeliefPropagation) {
        ...
        // naive extract
        naive_extract <<<normalize_grid, normalize_block>>>(cost_downsampling, inverse_depth,
                inverse_depth ? 1.0f / max_depth : min_depth, step_depth, image_cost.dev_ptr, debug_image.dev_ptr);
        cudaDeviceSynchronize();
        //printf("CUDA Status %s\n", cudaGetErrorString(cudaGetLastError()));
        if (printTimings)
        ...
        // debug_image.zero();
        bp_extract(cost_downsampling, inverse_depth, inverse_depth ? 1.0f / max_depth : min_depth, step_depth,
                   image_cost, debug_image, P1, P2);
...
void quadmap::SeedMatrix::download_output() {
    ...
    seq_output = income_seq;
#ifdef DEBUG_PRINT
    printf("download depth map cost %f ms \n", ( std::clock() - start ) / (double) CLOCKS_PER_SEC * 1000);start = std::clock();
#endif
}

void quadmap::SeedMatrix::get_result(cv::Mat &depth, cv::Mat &debug, cv::Mat &reference, cv::Mat &epipolar,
                                     cv::Mat &keyframe, float &seq) {
